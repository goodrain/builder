  ...
  context "on JDK 8" do
    ...
    it "deploy twice" do
      app.deploy do |app|
        ...
        expect(app.output).to include("Downloading https://mirrors.aliyun.com/macports/distfiles/gradle/gradle-")
        ...
        expect(app.output).to include("Building Gradle app")
        expect(app.output).to include("executing ./gradlew build -x test")
        expect(app.output).not_to include("Downloading https://mirrors.aliyun.com/macports/distfiles/gradle/gradle-")
        expect(app.output).not_to include(":test")
        expect(app.output).to include("BUILD SUCCESSFUL")
        expect(successful_body(app)).to eq("Hello from Spring Boot")
      end
